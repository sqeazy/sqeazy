...
namespace sqeazy
{
  ...
  template <typename T, typename S = std::size_t> struct lz4_scheme : public lz4_scheme_base_type<T>
  {
        ...
        , lz4_prefs()
    {
      ...
      lz4_prefs.compressionLevel = acceleration; /* compression level */
      lz4_prefs.autoFlush = 0;                   /* autoflush */
      lz4_prefs.favorDecSpeed = 0;
      
	  std::fill(lz4_prefs.reserved, lz4_prefs.reserved + 3,0u);
