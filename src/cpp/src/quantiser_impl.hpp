#ifndef __QUANTISER_IMPL_HPP__
#define __QUANTISER_IMPL_HPP__
...
namespace sqeazy {
  ...
  struct quantiser{
    ...
    static const typename sqeazy::twice_as_wide<raw_type>::type max_raw_ =  std::numeric_limits<raw_type>::max() + 1 ; 
    static const typename sqeazy::twice_as_wide<compressed_type>::type max_compressed_ = std::numeric_limits<compressed_type>::max() + 1; 

    static const typename sqeazy::twice_as_wide<raw_type>::type max_value_raw_ =  std::numeric_limits<raw_type>::max() ; 
    static const typename sqeazy::twice_as_wide<compressed_type>::type max_value_compressed_ = std::numeric_limits<compressed_type>::max(); 
	...
	//bucket overflow 
	if(quantile_sum >= bucketSize && comp_idx<max_compressed_){

	  lut_decode_[comp_idx] = static_cast<raw_type>(index_max_importance);
	  comp_idx++;
	  levels_available--;
	  
	  // if(!(comp_idx < max_compressed_)){
	  //   break;}
