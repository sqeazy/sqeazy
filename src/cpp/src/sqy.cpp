...
int main(int argc, char *argv[])
{
  ...
  descriptions["compress"].add_options()
    ...
    ("dataset_name,d", po::value<std::string>()->default_value("hokuspokus"), "name of the HDF5 dataset to appear inside any of .h5 encoded files (ignored for native .sqy compression)")
  ...
  descriptions["decompress"].add_options()
    ("help", "produce help message")
    ("verbose,v", "enable verbose output")
    ("dataset_name,d", po::value<std::string>()->default_value("hokuspokus"), "name of the HDF5 dataset to load from input h5 file(s) (ignored for native .sqy compression)")
    ("output_name,o", po::value<std::string>(), "file location to write output to (if only 1 is given)")
