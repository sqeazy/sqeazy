...
int forward_conversion(const bfs::path& _src,
  ...
  if(!(target_file_extension == ".y4m" || target_file_extension == ".yuv" || target_file_extension == ".raw")){
    std::cerr << "[SQY]\tunable to convert to anything else than y4m/yuv/raw (received  " << target_file_extension << ")!\n";
    return value;
  }
  ...
  //raw conversion (no quantisation supported yet)
  if(target_file_extension == ".raw"){

    std::ofstream ofile(_dst.generic_string(), std::ios::binary);
    ofile.write((char*)input.data(),input.size_in_byte());
    std::cout << _dst.generic_string() << ": " << input.bits_per_sample() << "bit (z,y,x)=(";
    for( std::uint32_t& el : input_dims )
      std::cout << el << " ";
    std::cout << ")\n";
    return value;
  }
