...
//   unsigned long max_size_compressed = sqeazy::lz4_pipe::static_max_bytes_encoded(retrieved.size()*sizeof(std::uint16_t),
// 										 hdr.size());
//   std::vector<char> compressed(max_size_compressed);
...
//   std::vector<std::uint16_t> decompressed(retrieved.size());
...
//   std::vector<std::uint16_t> to_store(64,42);
...
//   int rvalue = sqeazy::write_h5<std::uint16_t>(test_output_name.string(), dname, &constant_cube[0], dims);
...
//   rvalue = sqeazy::read_h5<std::uint16_t>(test_output_name.string(), dname, to_play_with, read_shape);
//   BOOST_REQUIRE_EQUAL(rvalue,0);
//   BOOST_REQUIRE_EQUAL_COLLECTIONS(dims.begin(), dims.end(),
