...
BOOST_AUTO_TEST_CASE( noisy_embryo_roundtrip ){
  ...
  std::vector<float> diff(noisy_embryo_.num_elements(),0);
  std::transform(reconstructed.begin(), reconstructed.end(),noisy_embryo_.data(),diff.begin(),
		 [](const uint16_t& _lhs,const uint16_t& _rhs){return float(float(_lhs) - float(_rhs));});

  double l2norm = std::inner_product(diff.begin(), diff.end(),diff.begin(),0.);

  BOOST_REQUIRE_GT(l2norm,0);
  l2norm = std::sqrt(l2norm)/noisy_embryo_.num_elements();
