...
BOOST_AUTO_TEST_CASE( double_in_one )
{
    std::vector<std::size_t> shape(dims.begin(), dims.end());
    ...
    std::size_t encoded_bytes = std::distance(encoded.data(),res);

    auto dbl = encoded;
    dbl.resize(2*encoded_bytes);
    std::copy(encoded.data(),res,
              dbl.begin());
    std::copy(encoded.data(),res,
              dbl.begin()+encoded_bytes);

    to_play_with.resize(2*to_play_with.size());
    auto rcode = local.decode(dbl.data(),
                              to_play_with.data(),
                              encoded_bytes,
                              size_in_byte*2);
    ...
    BOOST_REQUIRE_EQUAL_COLLECTIONS(
        ...
        incrementing_cube.end(),
        to_play_with.begin(),
        to_play_with.begin() + incrementing_cube.size()
        );
